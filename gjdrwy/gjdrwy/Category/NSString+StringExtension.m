...
+(NSString *)md5pwd:(NSString *)pwsstr{
    ...
    const int MD5_STR_LENGTH = 16;
    const char *cStr = [pwsstr UTF8String];
    unsigned char result[MD5_STR_LENGTH];
//    CC_MD5(cStr, strlen(cStr), result);
    CC_MD5(cStr, pwsstr.length, result);
    NSMutableString *md5passwd = [NSMutableString stringWithCapacity: MD5_STR_LENGTH];
    for(int i = 0; i < MD5_STR_LENGTH; i++) {
        [md5passwd appendFormat: @"%02X", result[i]];
    }
    md5str = md5passwd;
    return md5str;
}
- (CGRect) getStringRectWith:(int)maxwidth maxheight:(int)maxheight fontSize:(float)size
{
    ...
    return [self boundingRectWithSize: CGSizeMake(maxwidth, maxheight) options: NSStringDrawingUsesLineFragmentOrigin
                           attributes: @{
...
- (BOOL)isMobileNo
{
    ...
    NSString * CT = @"^1((33|53|8[093])[0-9]|349)\\d{7}$";
